...
export const caseContacts = pgTable("case_contacts", {
  ...
  isActive: boolean("is_active").notNull().default(true),
  // RTW Multi-Party Distribution (2026-05-27, phase 1):
  // Per-recipient distribution tracking. Columns hold "most-recent" state for
  // v1 (no re-distribution). When re-distribution lands, migrate to a sibling
  // rtw_plan_distributions table; these columns become "last_*" tracking.
...
// Multi-Party Distribution (2026-05-27, phase 1):
// Plan distribution state machine. Tracked on rtw_plans.distribution_status.
export type RTWDistributionStatus =
  | "not_distributed"      // Plan created; distribution not started.
  | "awaiting_responses"   // Emails sent; waiting for gating parties to respond.
  | "all_responded"        // All gating parties (worker/manager/doctor/physio-if-present) have responded.
  | "finalised";           // Practitioner marked complete after reviewing responses.
...
export const rtwPlans = pgTable("rtw_plans", {
  ...
  // RTW Multi-Party Distribution (2026-05-27, phase 1):
  // State machine for distributing the plan to worker/manager/doctor/physio/insurer.
  // Values: 'not_distributed' | 'awaiting_responses' | 'all_responded' | 'finalised'.
  // Approve-plan endpoint refuses transitions to 'approved' unless this is
  // 'all_responded' (or practitioner provides bypassReason — phase 2).
  distributionStatus: text("distribution_status").$type<RTWDistributionStatus>().notNull().default("not_distributed"),
